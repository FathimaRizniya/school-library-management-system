import { BookSelectInput } from '@/components/book-select-input';
...
const EditForm = () => {
    ...
    const { book } = usePage<{ props: { book: { id: string; title: string; author: string; isbn_no: string; category_id: string; cabinet_no: string; rack_no: string; available_copies: string; publication_year: string; publisher: string; book_price: string } } }>().props;

    const { data, setData, put, processing, errors, reset } = useForm<Required<BookForm>>({
        title: book.title,
        author: book.author,
        isbn_no: book.isbn_no,
        category_id: String(book.category_id),
        cabinet_no: book.cabinet_no,
            ...
            <form className="flex flex-col gap-6" onSubmit={submit}>
                <div className="grid gap-6 md:grid-cols-2">
                    ...
                    <div className="grid gap-2">
                        ...
                        {/* <BookSelectInput data={categories}
                            selected={data.category_name}
                            onSelect={(categories) => {
                            setData('category_id', categories.id);
                        }}  /> */}
